...
相较于面向过程是两种不同的处理问题的角度。

面向过程更注重于问题的每一个步骤以及准顺序，面向对象更注重于问题有哪些参与者（对象）、各自需要做什么。
...
注意到该 Employee 类没有什么不同，尽管该类是抽象类，但是它仍然有 3 个成员变量，7 个成员方法和 1 个构造方法。 现在如果尝试如下的例子：
...
用来描述类之间的继承关系，在 Java 中使用 extends 关键字。

![](/Users/hayden/Desktop/面试/Java/images/SoWkIImgAStDuU8goIp9ILLmJyrBBKh.png)
...
![](/Users/hayden/Desktop/面试/Java/images/SoWkIImgAStDuU8goIp9ILK8IatCoQn.png)
...
![](/Users/hayden/Desktop/面试/Java/images/SoWkIImgAStDuU8goIp9ILLmJ4ylIar.png)



##### 组合关系 (Composition)

组合关系中整体和部分是强依赖的，整体不存在了部分也就不存在了。例如公司和部门的关系，公司没了部门就不存在了。但是公司和员工就属于聚合关系了，因为公司没了员工还在。

![](/Users/hayden/Desktop/面试/Java/images/SoWkIImgAStDuU8goIp9ILLmpiyjo2_.png)



##### 关联关系 (Association)

表示不同类对象之间的关系，这是一种静态关系，与运行过程无关，在最开始就可以确定。因此也可以用 1:1，n:1，n:m 这种关联关系来表示。例如学校和学生就是一种关联关系，一个学校可以有多个学生，一个学生只属于一个学校，这是一种 n:1 的关系，在运行开始就可以确定。

![](/Users/hayden/Desktop/面试/Java/images/SoWkIImgAStDuU8goIp9ILLmB2xEJyv.png)



##### 依赖关系 (Dependency)

依赖关系是在运行过程中确定的。A 类和 B 类是依赖关系主要有三种形式：

1. A 类是 B 类中（某种方法）的局部变量
2. A 类是 B 类方法中的一个参数
...
以上的程序片段是合法定义的子接口，与类不同的是，接口允许多继承，而 Sports 及 Event 可以定义或是继承相同的方法。	
...
正如 EventListener 接口，这是由几十个其他接口扩展的 Java API，你可以使用一个标记接口来建立一组接口的父接口。例如：当一个接口继承了 EventListener 接口，Java 虚拟机就知道该接口将要被用于一个事件的代理方案。
...
继承是父类和子类之间共享数据和方法的机制。子类继承基类的方法，并做出自己的改变或扩展。
...
![2021112802](/Users/hayden/Desktop/面试/Java/images/2021112802.png)
...
覆盖是子类方法覆盖父类方法，以实现不同的功能或对父类的功能进行补充。



### 多态

在收到消息时，对象要予以响应。不同的对象在收到同一消息可以产生完全不同的结果，这一现象叫做多态。
...
![2021112803](/Users/hayden/Desktop/面试/之前的文件/pictures/Java工程师知识点/2021112803.jpeg)
